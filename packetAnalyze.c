...
void parsing(int* result, const unsigned char* netstr) {
    result[Version] = (netstr[IP_HEADER ])>>4;
    result[IHL] = ((netstr[IP_HEADER ])&0x0F) * 4;
    result[TOS] = netstr[IP_TOS] >> 4;
    result[TL] = (netstr[IP_TL] * 16 ) + netstr[IP_TL+1];
    result[Identification] = netstr[IP_IDENTIFICATION ] * 16 + netstr[IP_IDENTIFICATION +1];
    result[Flag] = netstr[IP_FRAGMENTATION] >> 5 ;
    result[FragmentOffset] = (netstr[IP_FRAGMENTATION] & 0x1f)*(0xff) + netstr[IP_FRAGMENTATION+1];
